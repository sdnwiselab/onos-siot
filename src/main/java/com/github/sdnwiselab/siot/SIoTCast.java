...
public class SIoTCast implements SIoTCastService {
    ...
    public void postSIoTCast(List<String> hostData) {
        ...
        try {
            ...
            log.info("CIAONEBIGBRO....."+e.toString());
            e.printStackTrace();
        }
    ...
    public void firewallFlowRule(HostId sourceHostId, Host destinationHost) {
        ...
        try {
            if (sourceHostIdString.equals(sourceHostId.toString())) {
                ...
                Host sourceHost = getHostById(sourceHostIdString);

                TrafficSelector selector = DefaultTrafficSelector.builder()
                        .matchEthDst(sourceHost.mac())
                        .build();
    ...
    //CREA LA FLOW RULE PER IL JOIN AD UN PUBLISHER
    public void joinPublisherFlowRule(Host sourceHost, String ipAddressGroup) {
        Device dev = deviceConnectedToHost(sourceHost.id().toString());
        List<Port> ports = new ArrayList<>();
        ports.addAll(deviceService.getPorts(dev.id()));
        PortNumber portNumber = null;
        for (Host ho : hostService.getConnectedHosts(dev.id())) {
            try {
                portNumber = mapHostPort.get(ho.id());
                ports.remove(deviceService.getPort(dev.id(), portNumber));
            } catch (Exception e) {
                ports.remove(deviceService.getPort(ho.location().deviceId(), ho.location().port()));
            }
        }
        ConnectPoint incp = new ConnectPoint(dev.id(), ports.get(1).number());
        ConnectPoint outcp = new ConnectPoint(dev.id(), sourceHost.location().port());


        TrafficSelector selector = DefaultTrafficSelector.builder()
                .matchEthType(Ethernet.TYPE_IPV4)
                .matchIPDst(IpPrefix.valueOf(ipAddressGroup + "/32"))
                .build();
        TrafficTreatment treatment = DefaultTrafficTreatment.builder()
                .setIpDst(IpAddress.valueOf("255.255.255.255"))
                .drop()
                .build();
        PointToPointIntent siotIntetn = PointToPointIntent.builder()
                .appId(appId)
                .priority(50000)
                .selector(selector)
                .treatment(treatment)
    ...
    //CREA LA FLOW RULE PER IL BLOCCO DEL MULTICAST DA HOST
    public void blockMulticast(Device dev) {

        for (Host ho : hostService.getConnectedHosts(dev.id())) {
            try {
                TrafficSelector selector = DefaultTrafficSelector.builder()
                        .matchEthType(Ethernet.TYPE_IPV4)
                        .matchIPDst(IpPrefix.valueOf("224.0.0.0/4"))
                        .matchInPort(ho.location().port())
                        .build();
                TrafficTreatment treatment = DefaultTrafficTreatment.builder()
                        .drop()
                        .build();
        ...
        public void process(PacketContext pc) {
            ...
            try {
                ...
                if (destIp.equals(mGroupCreationIp)) {
                    ...
                    if (firewall == false && publisher == false) {
                        ...
                        mapPublisherGroup.put(sourceHost, ipGroup);
                        ...
                        Device one = deviceService.getDevice(DeviceId.deviceId("of:0000000000000001"));
                        allDevice.add(one);
                    ...
                    if (publisher == true) {
                        log.info("RICEZIONE DEL PACCHETTO DI JOIN AD UN PUBLISHER: \n RELAZIONE " + relationship + " PER L'HOST " + sourceIp + " A DISTANZA " + hop);
                        HostId sourceHostId = null;
                        for (Map.Entry entry : hostIdentificationMap.entrySet()) {
                            if (sourceIp.equals(entry.getValue())) {
                                sourceHostId = HostId.hostId(entry.getKey().toString());
                        ...
                        for (String id : ids) {
                            Host ho = getHostById(id);
                            if (mapPublisherGroup.containsKey(ho)) {
                                createMulticastGroup(hostService.getHost(sourceHostId), mapPublisherGroup.get(ho));
                                joinPublisherFlowRule(hostService.getHost(sourceHostId), mapPublisherGroup.get(ho));
                            }
                        }
                        log.info("CREAZIONE REGOLA DI JOIN ESEGUITA");
                    }
                }
                else {
                    return;
